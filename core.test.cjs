...
test('les sept modules et les dix parcours ont des références et des identifiants uniques',()=>{
  assert.equal(modules.length,8);assert.equal(modules.flatMap(m=>m.variants).length,11);
  const ids=new Set();
...
test('distinction des référentiels et seuils critiques',()=>{
  ...
  assert.equal(get('vi').rows.length,5);
  assert.match(get('vi').rows.find(r=>r.score==='5').criteria,/extravésicale/);
  assert.match(get('vi').rows.find(r=>r.score==='3').action,/histologie/);
});
